...
- ~1505 `loadBudget`; ~1519 `renderBudget` — статьи ПО АЛФАВИТУ (order с исходными
  ...
  иерархия статья (brow) → группа работ (bgrp) → [подгруппа-поверхность bsub,
  только паркинг] → работа (bwork, можно много открытых) → сводная (bdetail,
  синяя заливка). Группы и подгруппы ПО УМОЛЧАНИЮ СВЕРНУТЫ (25.08:
  `budgetGrpOpen`/`budgetSubOpen` хранят РАСКРЫТЫЕ, не свёрнутые; поиск
  по работе раскрывает сам)
...
- ~48–53 ключи кэша (`floors_v3`, `vols_v1`, `budget_v21`, `bfloors_v1`, `changes_v1`,
