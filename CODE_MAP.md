...
- ~48–53 ключи кэша (`floors_v3`, `vols_v1`, `budget_v20`, `bfloors_v1`, `changes_v1`,
...
- ~818 `buildVolumes_`; ~935 `readLk_` («Личные_кабинеты» → [[МОЛ, статья,
  ...
  в ответ budget; группы = только «Раздел» — типы помещений, 25.08,
  разбивка на подразделы отменена; + зашитая группа «Не учтенные работы
