...
- `loadAnalytics`/`renderAnalytics` (после `renderBudget`, перед `alignBudgetHead`) —
  ...
  второй SVG «Помесячно по статьям» (26.08): пара столбцов на месяц (затраты/
  поступления), стек сегментов-статей (`perItem`/`orderK`/`colorOf` — палитра
  hsl по кругу 137,5°, крупные статьи первыми), подсказки на сегментах,
  компактная легенда `.an-items`
