...
- ~1505 `loadBudget`; ~1519 `renderBudget` — статьи ПО АЛФАВИТУ (order с исходными
  ...
  значения ТОЛЬКО в строках статей и Итого, МОЛ↔подрядчик по norm)·Закрытие
...
- `budgetSSHtml` (24.08) — сводная работ СС Шамова: МОЛ · Чел.-часы · Ставка ₽/час ·
  Стоимость (ячейки corp='СС'); работы паркинга (corp='Паркинг') не раскрываются
...
- ~48–53 ключи кэша (`floors_v3`, `vols_v1`, `budget_v13`, `bfloors_v1`, `changes_v1`,
...
- ~818 `buildVolumes_`; ~935 `readLk_` («Личные_кабинеты» → [[МОЛ, статья,
  ...
  `readShamov_` (лист «расчет_Шамов» → статья «Собственные силы (Шамов)»,
  группы = разделы, ячейки по МОЛам, сумма в cell[5], чел.-часы/ставка в
  cell[9]/[10], тоже `.concat`); затем `buildBudget_`
