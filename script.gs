var CONFIG = {
  ...
  FLOOR_MAT: 'Стоимость материалов на ед в бюджет',   // режим «Материалы» в сводной (04.08.2026)
...
var CACHE_FLOORS = 'floors_v3';   // сводка подрядчик × корпус + ssNames (см. action=floors); v3 — добавлена мат. расценка
 ...
 * Лист огромный (25 тыс. строк), поэтому читаем ТОЛЬКО нужные колонки и сразу
 * сворачиваем в агрегат: работа -> [[подрядчик, корпус, расценка, признак СС, мат. расценка], ...].
 * Мат. расценка — «Стоимость материалов на ед в бюджет» (режимы Работа/Материалы/Работа+материалы
 * в сводной, 04.08.2026); колонка опциональна — пропала из листа, значит в ячейках нули.
...
function buildFloorSummary_(ss) {
  ...
  var matCol = idx[CONFIG.FLOOR_MAT] ? col(CONFIG.FLOOR_MAT) : null;
  ...
  for (var k = 0; k < n; k++) {
    ...
    var mat = (matCol && typeof matCol[k][0] === 'number') ? matCol[k][0] : 0;

    if (!map[work]) map[work] = {};
    var key = contr + '' + corp;
    if (!map[work][key]) map[work][key] = [contr, corp, rate, isSS, mat];
    else {
      if (isSS) map[work][key][3] = 1;
      if (!map[work][key][4] && mat) map[work][key][4] = mat;
    }
  }
