var CONFIG = {
  ...
  // инициалом — на случай однофамильцев, как у Смирнова). Все прочие МОЛ
  // выгрузки берутся тоже, под именем «Фамилия И.О.» (см. m1cMol_).
...
/**
 * ФИО материально ответственного из 1С → имя МОЛ витрины ('' — МОЛ не указан).
 * Сначала — наши семь МОЛ из CONFIG.M1C_MOLS (короткие имена витрины), с
 * 11.09.2026 все остальные тоже берутся (решение пользователя «добавляем всех»)
 * под именем «Фамилия И.О.» — так «Дудин Александр Николаевич» и «Дудин А.Н.»
 * сходятся в одного МОЛ, а «Смирнов Владимир Андреевич» становится «Смирнов В.А.»
 * и не смешивается с нашим Смирновым (Александром).
 */
function m1cMol_(fio) {
  ...
  return m1cShortFio_(fio);
}

/** «Дудин Александр Николаевич» / «Дудин А.Н.» / «Базанов ВА» → «Дудин А.Н.». */
function m1cShortFio_(fio) {
  var parts = String(fio || '').trim().split(/\s+/).filter(function (p) { return p; });
  if (!parts.length) return '';
  if (parts.length === 1) return parts[0];
  var rest = parts.slice(1);
  var letters;
  if (/[а-яё]/.test(rest.join(''))) {
    // Полные имя и отчество — берём первые буквы.
    letters = rest.map(function (p) { return p.charAt(0); });
  } else {
    // Уже инициалы («А.Н.», «ВА») — берём все буквы.
    letters = rest.join('').replace(/[^A-Za-zА-ЯЁа-яё]/g, '').split('');
  }
  return parts[0] + ' ' + letters.map(function (c) { return c.toUpperCase() + '.'; }).join('');
}
...
function buildAnalytics_(ss) {
  ...
  if (shT && shT.getLastRow() > 1) {
    ...
    var colsT = [];   // [индекс колонки, месяц]
    for (var cT = CONFIG.TUZIO_MONTHS_START - 1; cT < dataT[0].length; cT++) {
      var hT = dataT[0][cT];
...
function readSsFact_() {
  var SS_FACT_SUM = 150000000;   // было 228 908 513, уточнение пользователя 25.08.2026
...
function readPark_(ss) {
  ...
  // Не учтённые в расчёте с заказчиком, но обязательные работы (итого
  // 34 212 885 руб — эксель «Доп работы по СБ3», данные пользователя
  // 25.08.2026). В листе «Паркинг» этих строк нет, а дописывать чужие листы
  // скрипту нельзя — зашиты здесь отдельной группой статьи «Паркинг».
  // Изменятся суммы/состав — править этот список. Разбивки на работы и
  // материалы нет — вся сумма в w.w («Работы, руб»), как у НР и Шамова.
  var extraGrp = 'Не учтенные работы в расчете с заказчиком';
  var extras = [
    ['Навигация (паркинг)', 12000000],
    ['Двери/шлагбаум', 9900000],
    ['Двери компакторная', 720000],
    ['Навигация (МОП, ЛК, кладовки)', 3360440],
    ['Отделка лифтовых порталов (МОП)', 8232444.8]
  ];
  var sep = String.fromCharCode(1);   // тот же невидимый разделитель, что в wKey
  if (!map['Паркинг']) map['Паркинг'] = { total: 0, works: {} };
  extras.forEach(function (e) {
    map['Паркинг'].total += e[1];
    map['Паркинг'].works[extraGrp + sep + e[0]] =
      { name: e[0], total: e[1], vol: 0, w: e[1], grp: extraGrp };
  });
...
function readShamov_(ss) {
  ...
  // Переименована из «Собственные силы (Шамов)» (просьба 25.08.2026).
  var ITEM = 'Подсоба Шамов';
  var total = 0;
  ...
  for (var k = 1; k < data.length; k++) {
    ...
    // Разделы «Общестрой» и «Тепловой контур» в статью не входят
    // (просьба 25.08.2026) — строки пропускаются целиком.
    var grpL = grp.toLowerCase();
    if (grpL === 'общестрой' || grpL === 'тепловой контур') continue;
